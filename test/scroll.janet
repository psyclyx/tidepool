...
(def w1 @{:wid 1})
(def l1 (tree/leaf w1))
(def result (scroll/layout-node l1 {:x 10 :y 20 :w 800 :h 600} ig bw 0 0))
...
(def l1 (tree/leaf w1))
(def l2 (tree/leaf w2))
(def c (tree/container :split @[l1 l2]))
(def result (scroll/layout-node c {:x 0 :y 0 :w 800 :h 600} ig bw 1 0))
...
(def l2 (tree/leaf w2))
(def c (tree/container :split @[l1 l2]))
(def result (scroll/layout-node c {:x 0 :y 0 :w 800 :h 600} ig bw 0 0))
(t/assert-eq (length result) 2)
...
(t/test-start "layout-node: horizontal split honors child ratios")
(tree/reset-ids)
(def w1 @{:wid 1})
(def w2 @{:wid 2})
(def l1 (tree/leaf w1 1.0))
(def l2 (tree/leaf w2 2.0))
(def c (tree/container :split @[l1 l2]))
(def result (scroll/layout-node c {:x 0 :y 0 :w 308 :h 100} ig bw 0 0))
# usable width = 300, weights 1:2 => 100 / 200
(t/assert-eq ((result 0) :w) 100)
(t/assert-eq ((result 1) :x) 108)
(t/assert-eq ((result 1) :w) 200)

(t/test-start "layout-node: 3 children, remainder distribution")
(tree/reset-ids)
(def leaves (seq [i :range [0 3]] (tree/leaf @{:wid i})))
(def c (tree/container :split leaves))
(def result (scroll/layout-node c {:x 0 :y 0 :w 100 :h 100} ig bw 1 0))
...
(t/test-start "layout-node: vertical split honors child ratios")
(tree/reset-ids)
(def w1 @{:wid 1})
(def w2 @{:wid 2})
(def l1 (tree/leaf w1 1.0))
(def l2 (tree/leaf w2 3.0))
(def c (tree/container :split @[l1 l2]))
(def result (scroll/layout-node c {:x 0 :y 0 :w 100 :h 408} ig bw 1 0))
# usable height = 400, weights 1:3 => 100 / 300
(t/assert-eq ((result 0) :h) 100)
(t/assert-eq ((result 1) :y) 108)
(t/assert-eq ((result 1) :h) 300)

(t/test-start "layout-node: tabbed places all children")
(tree/reset-ids)
(def w1 @{:wid 1})
(def w2 @{:wid 2})
(def l1 (tree/leaf w1))
(def l2 (tree/leaf w2))
(def tb (tree/container :tabbed @[l1 l2]))
(def result (scroll/layout-node tb {:x 0 :y 0 :w 800 :h 600} ig bw 0 0))
(t/assert-eq (length result) 2)
(t/assert-is ((result 0) :window) w1 "active=0 shows first")
(t/assert-eq ((result 0) :w) 800 "gets full rect")
(t/assert-is ((result 1) :window) w2 "inactive tab tracks position")
(t/assert-eq ((result 1) :w) 800 "inactive gets full rect too")
...
(def lb (tree/leaf wb))
(def lc (tree/leaf wc))
(def inner (tree/container :split @[lb lc]))
(def outer (tree/container :split @[la inner]))
(def result (scroll/layout-node outer {:x 0 :y 0 :w 800 :h 600} ig bw 1 0))
(t/assert-eq (length result) 3)
...
(t/test-start "scroll-layout: vertical split column (depth 1)")
(tree/reset-ids)
(def w1 @{:wid 1})
(def w2 @{:wid 2})
(def l1 (tree/leaf w1))
...
(t/assert-truthy (< (p1 :y) (p2 :y)) "first above second (depth 1 = vertical)")
